...
StateEngine:RegisterEvent("UNIT_SPELLCAST_SUCCEEDED") 
...
StateEngine:SetScript("OnEvent", function(self, event, unit)
    if event == "PLAYER_ENTERING_WORLD" then
        ...
        -- Restore saved cave entrance coordinates to persist darkness through reloads
        if UHCPM.IsDarkSubZone(GetMinimapZoneText()) then 
            state.isIndoors = true
            if UHCPM_Config and UHCPM_Config.savedEntranceX and UHCPM_Config.savedEntranceY then
                local distSq = ((px - UHCPM_Config.savedEntranceX) ^ 2) + ((py - UHCPM_Config.savedEntranceY) ^ 2)
                -- Teleport safety check: If you are more than 200 yards away from the saved entrance, reset it
                if distSq > 40000 then 
                    state.entranceX = px; state.entranceY = py
                    UHCPM_Config.savedEntranceX = px; UHCPM_Config.savedEntranceY = py
                else
                    state.entranceX = UHCPM_Config.savedEntranceX; state.entranceY = UHCPM_Config.savedEntranceY
                end
            else
                state.entranceX = px; state.entranceY = py
                if UHCPM_Config then UHCPM_Config.savedEntranceX = px; UHCPM_Config.savedEntranceY = py end
            end
        else 
    ...
    elseif (event == "ZONE_CHANGED_INDOORS" or event == "ZONE_CHANGED" or event == "ZONE_CHANGED_NEW_AREA") then
        ...
        if isDark and not state.isIndoors then 
            state.entranceX = px; state.entranceY = py; state.isIndoors = true
            -- Save coordinate state to config when entering a dark zone
            if UHCPM_Config then UHCPM_Config.savedEntranceX = px; UHCPM_Config.savedEntranceY = py end
        elseif not isDark and state.isIndoors then 
            state.isIndoors = false; state.entranceX = nil; state.entranceY = nil
            -- Wipe coordinate state when returning to the light
            if UHCPM_Config then UHCPM_Config.savedEntranceX = nil; UHCPM_Config.savedEntranceY = nil end
        end
    end
end)